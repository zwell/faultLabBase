...
先设定项目变量（默认可用 `basecamp`）：
  export FAULTLAB_PROJECT=${FAULTLAB_PROJECT:-basecamp}

创建第一个基于底座的场景 ${FAULTLAB_PROJECT}/scenarios/mysql/001-connection-pool/，
...
完成后运行：
  ...
  sh ${FAULTLAB_PROJECT}/scenarios/mysql/001-connection-pool/inject.sh
  sh ${FAULTLAB_PROJECT}/scenarios/mysql/001-connection-pool/test.sh
